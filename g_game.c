...
int PistolStarted = 1; //-Z
// [crispy] Write level statistics upon exit
static void G_WriteLevelStat(void)
{
    static FILE *fstream = NULL;

    int i, playerKills = 0, playerItems = 0, playerSecrets = 0, IsFast = 0, IsRespawn = 0, IsAlive = 1; //-Z
    ...
    //LEVEL#, SKILL, IsAlive?, PistolStarted?, KILLS, ITEMS, SECRETS, TIME, IsFast?, IsRespawn?
    //IsAlive?=0 dead, 1 alive/beaten the level
    //-Z

    if (players[consoleplayer].health <= 0) //dead
    {
    IsAlive = 0;
    }
    if (M_ParmExists("-fast"))
    {
    IsFast = 1;
    }
    if (M_ParmExists("-respawn"))
    {
    IsRespawn = 1;
    }
    fprintf(fstream, "%s,%d,%d,%d,%d/%d,%d/%d,%d/%d,%s,%d,%d\n",
            levelString, gameskill, IsAlive, PistolStarted, playerKills, totalkills, playerItems, totalitems, playerSecrets, totalsecret, levelTimeString, IsFast, IsRespawn); //-Z
}
...
{ 
	...
	// reload the level from scratch
        if (M_ParmExists("-levelstat")) //Save deaths to text file too -Z
        {
            G_WriteLevelStat();
        }
	PistolStarted = 1; //-Z
...
{ 
    PistolStarted = 0; //prevent loading confusing the pistol start varible -Z
